...
<main class="app-root"> <style> /* 이름 변경 모달 */ .overlay { position: fixed; inset: 0; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,0.45); backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); z-index: 1000; } .overlay[hidden] { display: none !important; } body.modal-open { overflow: hidden; } .modal { width: min(92vw, 420px); border-radius: 16px; padding: 20px; color: #fff; background: linear-gradient(180deg, rgba(20,28,42,0.85), rgba(20,28,42,0.68)); border: 1px solid rgba(255,255,255,0.22); backdrop-filter: blur(16px) saturate(120%); -webkit-backdrop-filter: blur(16px) saturate(120%); box-shadow: 0 20px 50px rgba(0,0,0,0.5); } .modal-title { margin: 0 0 6px; font-size: 20px; font-weight: 700; } .modal-desc { margin: 0 0 12px; opacity: .9; } .modal-input { width: min(9000px, 90%); height: 44px; padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.35); background: rgba(0,0,0,0.25); color: #fff; outline: none; } .modal-actions { display: flex; gap: 10px; justify-content: flex-end; margin-top: 16px; } .btn-primary, .btn-secondary { padding: 12px 16px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.28); box-shadow: 0 12px 28px rgba(0,0,0,.45); transition: background .2s ease, box-shadow .2s ease, transform .05s ease; cursor: pointer; color: #fff; backdrop-filter: blur(12px) saturate(140%); -webkit-backdrop-filter: blur(12px) saturate(140%); } .btn-primary { background: #20d07a; color: #0a1a12; font-weight: 700; border-color: rgba(0,0,0,0.15); } .btn-primary:hover { filter: brightness(1.05); box-shadow: 0 16px 32px rgba(0,0,0,.5); } .btn-primary:active, .btn-secondary:active { transform: translateY(1px); } .btn-secondary { background: rgba(255,255,255,0.14); border-color: rgba(255,255,255,0.28); } .btn-secondary:hover { background: rgba(255,255,255,0.22); } </style> <section id="auth-view" aria-live="polite" class="content"> <h2 id="panel-title" style="margin:0 0 12px;">로그인</h2> <form id="login-form"> <div> <input id="login-email" type="email" autocomplete="email" placeholder="이메일" required /> </div> <div> <input id="login-password" type="password" autocomplete="current-password" placeholder="비밀번호" required /> </div> <div> <button id="login-btn" type="submit">로그인</button> </div> </form>
<p id="to-signup">
  계정이 없나요?
...
</section> <header id="topbar" class="topbar" hidden> <div class="brand">TeTRIS</div> <div class="grow"></div> <div id="profile" class="profile" aria-haspopup="true"> <img id="profile-img" alt="프로필" class="avatar" /> <span id="profile-name" class="profile-name"></span> <button id="profile-toggle" class="profile-toggle" aria-label="프로필 메뉴" aria-expanded="false">▾</button> <div id="profile-menu" class="profile-menu" hidden> <button id="change-name" type="button">이름 변경</button> <button id="change-photo" type="button">프로필 이미지 변경</button> <button id="logout-btn" type="button">로그아웃</button> </div> <input id="photo-input" type="file" accept="image/*" hidden /> </div> </header> <section id="content-area" class="content-area" hidden></section> <!-- 이름 변경 모달 --> <div id="name-modal-overlay" class="overlay" hidden> <div class="modal" role="dialog" aria-modal="true" aria-labelledby="name-modal-title"> <h3 id="name-modal-title" class="modal-title">이름 변경</h3> <p class="modal-desc">표시할 이름을 입력하세요.</p> <input id="name-input" type="text" class="modal-input" maxlength="40" /> <div class="modal-actions"> <button id="name-cancel" type="button" class="btn-secondary">취소</button> <button id="name-save" type="button" class="btn-primary">완료</button> </div> </div> </div> </main> `;
...
// 게임 시작 → 멀티 플레이 화면으로 전환(방장)
renderMultiplayerGame(roomRef, roomCode, true);
...
renderMultiplayerGame(roomRef, roomCode);
} else if (data.status === "closed") {
statusEl.textContent = "방이 종료되었습니다.";
renderMultiEntry();
} else if (data.status === "finished") {
...
/* ===== 멀티플레이 게임 화면 ===== */
function renderMultiplayerGame(roomRef, roomCode, isHost = false) {
const user = auth.currentUser;
...
contentArea.innerHTML = `
<style>
.mp-stage-wrap { min-height: calc(100vh - 64px); display:flex; align-items:center; justify-content:center; }
.mp-stage { position: relative; width: min(96vw, 1100px); height: min(88vh, 720px); }
.mp-top { position:absolute; top:-44px; left:0; right:0; display:flex; align-items:center; justify-content:space-between; color:#fff; text-shadow:0 2px 8px rgba(0,0,0,.6); }
.mp-btn { background: rgba(255,255,255,0.14); border:2px solid rgba(0,0,0,0.9); color:#fff; border-radius:10px; padding:8px 10px; cursor:pointer; box-shadow:0 8px 16px rgba(0,0,0,.35); }
.mp-title { font-weight:800; letter-spacing:.5px; }
.play-grid { position:absolute; left:50%; transform:translateX(-50%); top:0; bottom:0; width:min(54vw, 420px); display:flex; align-items:center; justify-content:center; padding:12px; }
.field {
height: 100%; aspect-ratio: 10 / 20;
background-color: rgba(20,28,42,0.50);
backdrop-filter: blur(10px) saturate(140%); -webkit-backdrop-filter: blur(10px) saturate(140%);
border: 3px solid rgba(0,0,0,0.95);
box-shadow: 0 10px 24px rgba(0,0,0,.35), inset 0 0 0 2px rgba(255,255,255,0.08);
...
.right-rail { position:absolute; right:0; top:6%; width:min(18vw, 220px); height:76%; overflow:auto; display:flex; flex-direction:column; gap:10px; }
.mp-card { border:1px solid rgba(255,255,255,0.22); border-radius:10px; padding:8px; background: rgba(0,0,0,0.25); color:#fff; }
.mp-name { font-weight:800; margin-bottom:6px; }
.mp-mini { width:100%; }
.field-mini { width:100%; aspect-ratio:10/20; }
.mp-me { position:absolute; top:6%; left:0; width:min(20vw, 180px); }
.mp-me-name { color:#fff; font-weight:800; text-shadow:0 2px 8px rgba(0,0,0,.55); }
@media (max-width: 860px) {
...
<div class="mp-stage-wrap">
  <div class="mp-stage" role="region" aria-label="멀티플레이 게임">
    <div class="mp-top">
      <button id="mp-leave" type="button" class="mp-btn">나가기</button>
      <div class="mp-title">멀티 플레이 — <span id="mp-room-code">${roomCode}</span></div>
      <div>
        ${isHost ? '<button id="mp-finish" type="button" class="mp-btn">게임 종료</button>' : '<span style="display:inline-block; width:84px;"></span>'}
      </div>
    </div>

    <div class="mp-me" aria-hidden="true">
      <div class="mp-me-name" id="mp-me-name"></div>
    </div>

    <div class="play-grid">
      <div class="field" id="mp-main-field" role="img" aria-label="내 10×20 필드"></div>
    </div>

    <div class="right-rail" id="mp-previews" aria-label="다른 플레이어 미니 보드"></div>
  </div>
</div>
`;

const leaveBtn = document.getElementById("mp-leave");
const finishBtn = document.getElementById("mp-finish");
const previewsEl = document.getElementById("mp-previews");
const meNameEl = document.getElementById("mp-me-name");
if (meNameEl) meNameEl.textContent = fallbackName(user);
...
for (const [uid, el] of previews) {
if (!uids.has(uid)) { el.remove(); previews.delete(uid); }
}
list.forEach(p => {
let card = previews.get(p.uid);
if (!card) {
card = document.createElement("div");
card.className = "mp-card";
card.innerHTML = <div class="mp-name"></div> <div class="mp-mini"><div class="field field-mini" role="img" aria-label="플레이어 10×20 미니 보드"></div></div>;
previews.set(p.uid, card);
previewsEl.appendChild(card);
}
const nameEl = card.querySelector(".mp-name");
if (nameEl) nameEl.textContent = (p.name || "플레이어") + (p.isHost ? " (방장)" : "");
card.style.display = p.eliminated ? "none" : "";
});
}
...
snap.forEach((d) => {
const p = d.data();
if (p.uid === mine) meDoc = p; else others.push(p);
});
others.sort((a, b) => (b.isHost ? 1 : 0) - (a.isHost ? 1 : 0));
if (meDoc && meNameEl) meNameEl.textContent = meDoc.name || meNameEl.textContent;
syncPreviews(others);
});
...
// 호스트: 게임 종료(시작 전 화면 복귀)
finishBtn?.addEventListener("click", async () => {
try { await updateDoc(roomRef, { status: "finished", finishedAt: serverTimestamp() }); } catch {}
});
}
...
const bg = "#2be47f";
const fg = "#0a1a12";
const svg = <svg xmlns='http://www.w3.org/2000/svg' width='64' height='64' viewBox='0 0 64 64'> <rect width='64' height='64' rx='32' ry='32' fill='${bg}'/> <text x='50%' y='50%' dy='.36em' text-anchor='middle' font-size='36' font-family='system-ui, -apple-system, Segoe UI, Roboto, Noto Sans, Apple SD Gothic Neo, Malgun Gothic, Arial, sans-serif' font-weight='700' fill='${fg}'>${ch}</text> </svg>;
